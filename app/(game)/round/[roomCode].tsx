...
export default function RoundScreen() {
  ...
  const initLocalAnswers = useRoundStore((s) => s.initLocalAnswers);
  ...
  // Initialize local answers once per round (keyed on roundIndex).
  // submitted guards against re-initialising after the player submits.
  useEffect(() => {
    if (room && round && !submitted) {
      initLocalAnswers(room.categories);
    }
    // Intentionally depends only on roundIndex so that state updates to
    // `room` or `submitted` mid-round do not reset the answers the player
    // is actively typing. initLocalAnswers is a stable Zustand setter.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [round?.roundIndex]);

  // Check if already submitted (e.g. reconnection)
  useEffect(() => {
  ...
  return (
    ...
    >
      {/* Header: Round info + Letter */}
      <View style={styles.header}>
        <Text style={styles.roundInfo}>
          {t('round.round', { current: round.roundIndex + 1, total: room.totalRounds })}
        </Text>
        <Text style={styles.letterLabel}>{t('round.fill_in_words')}</Text>
        ...
        {categories.map((cat, i) => {
          ...
          return (
            ...
            >
              ...
              <Text
                style={[
                  styles.overviewAnswer,
                  !answer && styles.overviewEmpty,
                ]}
                numberOfLines={1}
              >
                {answer || '—'}
